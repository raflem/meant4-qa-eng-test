...
def save_state(function_browser, state):
    """Helper function"""
    visible_products = tuple(function_browser.find_by_css(".product-container"))
    visible_prices = tuple(function_browser.find_by_css(".price.product-price"))
...
def open_shop_site(function_browser):
    function_browser.visit("http://www.automationpractice.pl/index.php?id_category=8&controller=category")


@given("Default filters and sorting are enabled")
def save_default_state(function_browser, default_state):
    visible_products = tuple(function_browser.find_by_css(".product-container"))
    visible_prices = tuple(function_browser.find_by_css(".price.product-price"))
...
def enable_filters(function_browser, filtered_state):
    function_browser.find_by_id("layered_category_11").click()
...
def change_sorting(function_browser, sorted_state):
    function_browser.find_by_id("selectProductSort").select("price:desc")
    time.sleep(1.5)  # force wait to avoid staleness
    visible_products = tuple(function_browser.find_by_css(".product-container"))
    visible_prices = tuple(function_browser.find_by_css(".price.product-price"))
